...
يهدف هذا المعجم إلى تقديم بعض الإرشادات حول معاني المصطلحات الفنية الشائعة الاستخدام عند الحديث عن Vue. والغرض من هذا المعجم هو أن يكون *وصفيًا* لكيفية استخدام المصطلحات بشكل شائع، وليس تحديدًا *إرشاديًا* لكيفية استخدامها. قد يكون لبعض المصطلحات معاني أو فروق دقيقة مختلفة قليلاً حسب السياق المحيط.

[[TOC]]

## مكون غير متزامن {#async-component}

*المكون غير المتزامن* عبارة عن غلاف حول مكون آخر يسمح بتحميل المكون المغلف بشكل خامل. يستخدم هذا عادةً كطريقة لتقليل حجم ملفات `.js` المبنية، مما يسمح بتقسيمها إلى أجزاء أصغر يتم تحميلها فقط عند الحاجة.

يحتوي Vue Router على ميزة مماثلة [للتحميل الخامل لمكونات المسار](https://router.vuejs.org/guide/advanced/lazy-loading.html)، على الرغم من أن هذا لا يستخدم ميزة مكونات Vue غير المتزامنة.
...
*ماكرو المصرف* هو شيفرة خاص تتم معالجته بواسطة المصرف وتحويله إلى شيء آخر. إنها في الواقع شكل ذكي من أشكال استبدال السلسلة نصية.

يدعم المصرف [SFC](#single-file-component) من Vue وحدات ماكرو مختلفة، مثل `defineProps()` و`defineEmits()` و`defineExpose()`. تم تصميم وحدات الماكرو هذه عمدًا لتبدو مثل دوال JavaScript العادية حتى تتمكن من الاستفادة من نفس أدوات التحليل و استنباط النوع حول JavaScript / TypeScript. ومع ذلك، فهي ليست دوال فعلية يتم تشغيلها في المتصفح. هذه هي سلاسل خاصة يكتشفها المصرف ويستبدلها بشيفرة JavaScript الحقيقي الذي سيتم تشغيله بالفعل.

وحدات الماكرو لها قيود على استخدامها لا تنطبق على شيفرة JavaScript العادي. على سبيل المثال، قد تعتقد أن `const dp = defineProps` يسمح لك بإنشاء اسم مستعار لـ `defineProps`، لكن هذا سيؤدي في الواقع إلى حدوث خطأ. هناك أيضًا قيود على القيم التي يمكن تمريرها إلى `defineProps()`، حيث يتعين معالجة 'المعاملات' بواسطة المصرف وليس في وقت التشغيل.
...
تحدد خيارات المكون كيفية إنشاء نسخ لهذا المكون. المكونات تشبه مفاهيميًا الفئات، على الرغم من أن Vue لا تستخدم فئات JavaScript الفعلية لتعريفها.

يمكن أيضًا استخدام مصطلح المكون بشكل أكثر مرونة للإشارة إلى نسخ المكون.
...
* عادةً ما تعيد composables كائنًا عاديًا، وليس كائنًا تفاعليًا. عادةً ما يحتوي هذا الكائن على مراجع ودوال ومن المتوقع أن يتم تفكيكه داخل الشيفرة المستدعي.

كما هو الحال مع العديد من الأنماط، قد يكون هناك بعض الخلاف حول ما إذا كان الشيفرة المحدد مؤهلاً للتسمية. ليست كل دوال الأداة المساعدة في JavaScript قابلة للتكوين. إذا كانت الوظيفة لا تستخدم واجهة برمجة التطبيقات للتكوين، فربما لا تكون قابلة للتكوين. إذا لم تكن تتوقع استدعاؤها أثناء التنفيذ المتزامن لـ `setup()`، فربما لا تكون قابلة للتكوين. تُستخدم العناصر القابلة للتكوين على وجه التحديد لتغليف المنطق ذي الحالة، وهي ليست مجرد اتفاقية تسمية للدوال.
...
*Composition API* عبارة عن مجموعة من الدوال المستخدمة لكتابة المكونات والعناصر القابلة للتكوين في Vue.
...
يدعم Vue أيضًا إنشاء توجيهات مخصصة، على الرغم من أنها تُستخدم عادةً فقط كـ "منفذ هروب" للتعامل مع عقد DOM بشكل مباشر. لا يمكن استخدام التوجيهات المخصصة عمومًا لإعادة إنشاء دوال التوجيهات المضمنة.
...
تُستخدم المقطع لدعم المكونات ذات العقد الجذرية المتعددة. وبينما قد تبدو هذه المكونات وكأنها تحتوي على جذور متعددة، فإنها تستخدم وراء الكواليس عقدة شظية كجذر واحد، كأصل للعقد "الجذرية".

تُستخدم المقطع أيضًا بواسطة مُجمِّع القالب كطريقة لتغليف عقد ديناميكية متعددة، على سبيل المثال تلك التي تم إنشاؤها عبر `v-for` أو `v-if`. يسمح هذا بتمرير تلميحات إضافية إلى خوارزمية التصحيح [VDOM](#virtual-dom). يتم التعامل مع الكثير من هذا داخليًا، ولكن هناك مكان قد تواجه فيه هذا مباشرةً وهو استخدام `key` على علامة `<template>` مع `v-for`. في هذا السيناريو، تتم إضافة `key` كـ [prop](#prop) إلى عقدة VNode المجزأة.
...
*(functional component)* هو شكل بديل للمكون يتم إعلانه باستخدام دالة بدلاً من ذلك. تعمل هذه الدالة كـ [دالة تشكيل](#render-function) للمكون.
...
لمزيد من التفاصيل، راجع:
- [الدليل - دوال التقديم وJSX - المكونات الوظيفية](/guide/extras/render-function.html#functional-components)
...
يستخدم JavaScript عملية (hoisting) لبعض البنيات، مثل `var` و`import` وإعلانات الدوال.
...
توجد طرق مختلفة لتحديد قالب لمكون. في أغلب الحالات، يتم توفير القالب كسلسلة نصية.

يشير مصطلح *قالب داخل DOM* إلى السيناريو الذي يتم فيه توفير القالب في هيئة عقد DOM، بدلاً من سلسلة نصية. ثم يقوم Vue بتحويل عقد DOM إلى سلسلة نصية قالب باستخدام `innerHTML`.
...
تمر نسخ مكون Vue بدورة حياة. على سبيل المثال، يتم إنشاؤها وتركيبها وتحديثها وإلغاء تركيبها.

*(lifecycle hooks)* هي طريقة للاستماع إلى أحداث دورة الحياة هذه.

باستخدام واجهة برمجة التطبيقات Options، يتم توفير كل خطاف كخيار منفصل، على سبيل المثال `mounted`. تستخدم واجهة برمجة التطبيقات Composition دوال بدلاً من ذلك، مثل `onMounted()`.

لمزيد من التفاصيل، راجع:
- [دليل - (lifecycle hooks)](/guide/essentials/lifecycle.html)

## ماكرو {#macro}

انظر [ماكرو المصرف](#macro-compiler).
...
## واجهة الخيارات البرمجية API {#options-api}
...
في حين يمكن استخدام مصطلح *plugin* في مجموعة متنوعة من السياقات، فإن Vue لديه مفهوم محدد للملحق كطريقة لإضافة وظيفة إلى تطبيق.

تتم إضافة المكونات الإضافية إلى التطبيق عن طريق استدعاء `app.use(plugin)`. الملحق نفسه عبارة عن دالة أو كائن به دالة `install`. سيتم تمرير هذه الدالة إلى مثيل التطبيق ويمكنها بعد ذلك القيام بكل ما يلزم القيام به.
...
هناك ثلاثة استخدامات شائعة لمصطلح *خاصية* في Vue:

* خاصية تجمع على خاصيات
* خصائص VNode
* خصائص الفتحة
...
يشير مصطلح *خصائص VNode* إلى خصائص الكائن التي يتم تمريرها كحجة ثانية إلى `h()`. يمكن أن تتضمن هذه خصائص المكوّن، ولكنها يمكن أن تتضمن أيضًا أحداث المكوّن وأحداث DOM وسمات DOM وخصائص DOM. عادةً ما تواجه خصائص VNode فقط إذا كنت تعمل مع دوال العرض للتلاعب بعقد VNodes مباشرةً.
...
- [دليل - دوال العرض وJSX](/guide/extras/render-function.html)
...
هناك طرق مختلفة يمكن من خلالها تنفيذ نظام التفاعلية. على سبيل المثال، يمكن القيام بذلك من خلال التحليل الثابت للشيفرة لتحديد تبعياته. ومع ذلك، لا يستخدم Vue هذا الشكل من نظام التفاعلية.

بدلاً من ذلك، يتتبع نظام التفاعلية في Vue الوصول إلى الخصائص في وقت التشغيل. وهو يفعل ذلك باستخدام كل من غلافات الوكيل ودوال الحصول/التعيين للخصائص.
...
## الواجهة البرمجية التفاعلية {#reactivity-api}

*واجهة برمجة تطبيقات التفاعل* عبارة عن مجموعة من دوال Vue الأساسية المرتبطة بـ [reactive](#reactiveity). ويمكن استخدامها بشكل مستقل عن المكونات. وهي تتضمن دوال مثل `ref()` و`reactive()` و`computed()` و`watch()` و`watchEffect()`.
...
## دالة التشكيل {#render-function}

*دالة التشكيل* هي جزء من أحد المكونات التي تولد العقد الافتراضية المستخدمة أثناء العرض. يتم تجميع القوالب إلى دوال عرض.

لمزيد من التفاصيل، راجع:
- [الدليل - دوال العرض وJSX](/guide/extras/render-function.html)

## (scheduler) {#scheduler}

*(scheduler)* هو جزء من مكونات Vue الداخلية التي تتحكم في توقيت تشغيل [التأثيرات التفاعلية](#reactive-effect).
...
[Watchers](/guide/essentials/watchers.html) يتم تجميعها أيضًا باستخدام قائمة انتظار (scheduler). سيتم تشغيل المراقبين الذين لديهم `flush: 'pre'` (الافتراضي) قبل عرض المكون، بينما سيتم تشغيل المراقبين الذين لديهم `flush: 'post'` بعد عرض المكون.

تُستخدم الدوال في (scheduler) أيضًا لأداء مهام داخلية أخرى مختلفة، مثل تشغيل بعض [lifecycle hooks](#lifecycle-hooks) وتحديث [template refs](#template-ref).
...
لا يقتصر مصطلح *التأثير الجانبي* على Vue. فهو يُستخدم لوصف العمليات أو الدوال التي تقوم بشيء يتجاوز نطاقها المحلي.
...
## المكون أحادي الملف {#single-file-component}
